...
int main_ping_pong_1players() {
    ...
    int length_palette = 8;
    std::wstring      strPlayer(length_palette, 0x2588);
    cgu::DynSprite    player1{ strPlayer, length_palette ,1, 4, y_extLine2 - 1 };
...
ESC: Quite Game)";
    ...
    auto reset_ball = [&] {
        rng::fRG<float> frand;
        rng::iRG<int>   irand;
        ball.set_position(frand(4.f, 35.f), 5);
        //ball.reverse(irand(0, 2));
        // ball time
        ball_times = ball_timer.get_elapsed_time();
        ball_timer.reset();
...
int main() {
        ...
        main_ping_pong_1players();
     //   main_ping_pong_2players();
