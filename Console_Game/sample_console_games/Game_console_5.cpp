...
S:Sellect R:Desellect U:Speeding D:Unspeeding I:Pause)";
	...
	while (true) {
		...
		if (KeyReleased(_u('S'))) {
			std::vector<std::wstring> vwstr{ L"\x2588\x2588\x2588" , L"\x2588\x2588\x2588" , L"\x2588\x2588\x2588" };
			vboxes.emplace_back( vwstr , 20.f, 20.f);
			vboxes[vboxes.size() - 1].set_id(++num_id);
			vboxes[vboxes.size() - 1].set_controle_key();
			ptr = &vboxes[vboxes.size() - 1];
		}
